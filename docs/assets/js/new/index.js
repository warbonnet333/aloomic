...
window.onload = function () {
    const images = document.querySelectorAll('.with-custom-scroll');
    if (window.innerWidth <= 768 || !images.length) return;
    let mouse = {
        x: -100,
        y: -100
    };
    ...
    const cursor = document.querySelector('.with-custom-scroll-inner');

    new CircleType(cursor).radius(44);
    ...
    images.forEach(function (item) {
        item.setAttribute('data-left', 0);
        item.setAttribute('data-top', 0);
        item.addEventListener('pointermove', () => requestAnimationFrame(updateCursor));
        item.addEventListener('pointermove', showCursor);
        item.addEventListener('pointerleave', removeCursor);
    });
}
